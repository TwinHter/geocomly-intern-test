...
func run() error {
	...
	support := flag.String("support", "", "average or sum cluster support")
	flag.Parse()
	...
	if *clustersPath != "" {
		...
		switch *support {
		case "", "average":
			config.NormalizeClusterSupport = true
		case "sum":
			config.NormalizeClusterSupport = false
		default:
			return fmt.Errorf("unsupported support %q; expected average or sum", *support)
		}

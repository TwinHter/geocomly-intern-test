...
type metrics struct {
	...
	AffectedLegitActors      int                `json:"affected_legitimate_actors"`
...
func run() error {
	...
	threshold := flag.Float64("threshold", 0, "override raw link-evidence threshold with --accounts")
	strongThreshold := flag.Float64("strong-threshold", 0, "override strong raw-evidence threshold")
	...
	if *clustersPath != "" {
		...
		if flagWasSet("threshold") {
			config.LinkEvidenceThreshold = *threshold
		}
		if flagWasSet("strong-threshold") {
			config.StrongEvidenceThreshold = *strongThreshold
		}
		if *linkage != "" {
	...
	if len(accounts) > 0 {
		...
		result.FraudPairScores = fraudPairScores(accounts, truth, scorer.RawScore)
	}
